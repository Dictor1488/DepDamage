package net.wg.gui.battle.views.vehicleMarkers
{
   ...
   public class VehicleStatusContainerMarker extends BattleUIComponent
   {
      ...
      {
         return this._activeEffectID == -1 ? false : Boolean(this.getMarker(this._activeEffectID).isVisible());
      }
      ...
      {
         var _loc9_:VehicleAnimatedStatusBaseMarker = null;
         var _loc10_:VehicleAnimatedStatusBaseMarker = null;
         if(param2 > param6)
         {
            return;
         }
         var _loc11_:VehicleAnimatedStatusBaseMarker = this.getMarker(param1);
         var _loc12_:Boolean = false;
         if(this._activeEffectID != -1 && this._activeEffectID != param1)
         {
            _loc9_ = this.getMarker(this._activeEffectID);
            if(_loc9_ != _loc11_)
            {
            ...
            if(param1 != param5)
            {
               ...
               {
                  _loc10_ = this.getMarker(this._oneShotStatusID);
                  if(_loc10_)
                  {
                     _loc10_.resetMarkerStates();
                  }
               }
               if(_loc11_)
               {
         ...
         if(_loc11_)
         {
            if(_loc11_.isAtlasSrcMode())
            {
               _loc11_.setStatusID(param1);
               _loc11_.updateAssets();
            }
            _loc11_.showEffectTimer(param4,param3,_loc12_,param7,param8);
         }
