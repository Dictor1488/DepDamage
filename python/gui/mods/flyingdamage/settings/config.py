...
class Config(object):
    ...
    def __init__(self):
        ...
        self.fontSize = 24
        self.opacity = 100
        # Native VehicleMarker path must not hide the standard marker damage:
        # the original addDamageLabel() is exactly the code we want to use.
        self.hideStandard = False
        self.hideMyDamage = False

        self.anchorMode = 'native_vehicle_marker'
        self.risePixels = 55
    ...
    def _template(self):
        ...
        return {
            ...
            'column1': [
                {'type': 'Label', 'text': u'Render mode: native VehicleMarker.addDamageLabel'},
                {'type': 'Label', 'text': u'Damage is drawn inside the real tank marker, not in FlyingDamageApp overlay.'},
                {'type': 'Slider', 'text': u'Text size',
                 'value': self.fontSize, 'minimum': 12, 'maximum': 48,
                 'step': 1, 'format': u'{{value}} px', 'varName': 'fontSize'},
                {'type': 'CheckBox', 'text': u'Color by team (legacy setting)',
                 'value': self.colorByTeam, 'varName': 'colorByTeam'},
                dropdown(u'Enemy color (legacy setting)', self.enemyColorIndex, 'enemyColorIndex'),
                dropdown(u'Ally color (legacy setting)', self.allyColorIndex, 'allyColorIndex'),
                dropdown(u'Single color (legacy setting)', self.colorIndex, 'colorIndex'),
            ],
            'column2': [
                {'type': 'Label', 'text': u'Native marker damage is not suppressed.'},
                {'type': 'Slider', 'text': u'Opacity (legacy setting)',
                 'value': self.opacity, 'minimum': 0, 'maximum': 100,
                 'step': 5, 'format': u'{{value}} %', 'varName': 'opacity'},
                {'type': 'Slider', 'text': u'Rise height (legacy setting)',
                 'value': self.riseMeters, 'minimum': 0.4, 'maximum': 3.0,
                 'step': 0.05, 'format': u'{{value}} m', 'varName': 'riseMeters'},
                {'type': 'Slider', 'text': u'Lifetime (legacy setting)',
    ...
    def _apply(self, s):
        try:
            ...
            # Ignore old saved values. Native marker labels must remain visible.
            self.hideStandard = False
            self.hideMyDamage = False
